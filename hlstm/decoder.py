...
class SentenceRNN(nn.Module):
    ...
    def forward(self, pooling_vector, states=None):
        pooling_vector = pooling_vector.unsqueeze(1)            # (batch_size, 256) -> (batch_size, 1, 256)

        hiddens, states = self.lstm(pooling_vector, states)     # hiddens = (batch_size, 1, 256)
                                                                # states (hn, cn) = (1, batch_size, 256)

        endToken = self.endTokenActivation(self.endTokenLinear(hiddens))    # (batch_size, 1, 256) -> (batch_size, 1, 1)
        # endToken = endToken >= 0.5
        endToken = endToken.type(torch.float)
        topicVector = self.topicVectorActivation(self.topicVectorLinear(hiddens))   # (batch_size, 1, 256) -> (batch_size, 1, 256)
...
#         self.embedding = nn.Embedding.from_pretrained(wordVectors)
    ...
    def forward(self, enc_image, global_features, encoded_captions, caption_lengths):
        ...
        spatial_image = F.relu(self.encoded_to_hidden(enc_image))           # (bs, num_pixels, hidden_size)
        global_image = F.relu(self.global_features(global_features))        # (bs, embed_size)
